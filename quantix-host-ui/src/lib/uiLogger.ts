/**
 * UI Action Logger for Quantix-OS Host UI (QHCI)
 * 
...
export const COMPONENT_CONFIG: Record<UIComponent, ComponentConfig> = {
  ...
  host: { icon: 'Server', color: '#3b82f6', label: 'Host', category: 'infrastructure' },
  ...
  hardware: { icon: 'Cpu', color: '#06b6d4', label: 'Hardware', category: 'infrastructure' },
  certificates: { icon: 'Key', color: '#ec4899', label: 'Certificates', category: 'security' },
  registration: { icon: 'UserPlus', color: '#14b8a6', label: 'Registration', category: 'cluster' },
};
...
export function getSessionId(): string {
  if (!sessionId) {
    ...
    sessionId = sessionStorage.getItem('quantix-host-session-id');
    if (!sessionId) {
      sessionId = generateSessionId();
      sessionStorage.setItem('quantix-host-session-id', sessionId);
    }
  }
  return sessionId;
}
