...
/// Parse a qvmc:// URL into connection parameters
fn parse_qvmc_url(url: &str) -> Option<PendingConnection> {
    // Format: qvmc://connect?url=<encoded>&vmId=<id>&vmName=<name>
    // Also handle: qvmc://connect/?url=... (with trailing slash)
    
    // Try both formats
    let query = url
        .strip_prefix("qvmc://connect?")
        .or_else(|| url.strip_prefix("qvmc://connect/?"))?;
...
fn handle_deep_link(app: &AppHandle, url: &str) {
    ...
    if let Some(conn) = parse_qvmc_url(url) {
...
fn debug_log(msg: &str) {
    ...
    if let Some(home) = dirs::home_dir() {
        let log_path = home.join("qvmc-debug.log");
...
fn main() {
    ...
    info!("Starting qvmc v{}", env!("CARGO_PKG_VERSION"));
    debug_log(&format!("Starting qvmc v{}", env!("CARGO_PKG_VERSION")));
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            debug_log("qvmc initialized successfully");
            info!("qvmc initialized successfully");
            Ok(())
        })
        .run(tauri::generate_context!())
        .expect("Error running qvmc");
}
