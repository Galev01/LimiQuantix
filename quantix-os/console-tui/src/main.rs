...
fn apply_static_ip(config: &StaticIpConfig) -> Result<()> {
    ...
    // The quantix-network service reads from /etc/quantix/network/
    let config_dir = "/etc/quantix/network";
    let _ = std::fs::create_dir_all(config_dir);
    
    // Save interface-specific config
    let interface_config = format!(
        "# Static IP configuration for {}\n# Generated by Quantix Console\nIP_ADDRESS={}\nNETMASK={}\nGATEWAY={}\nDNS={}\n",
        config.interface, config.ip_address, config.netmask, config.gateway, config.dns
    );
    let interface_config_path = format!("{}/{}.conf", config_dir, config.interface);
    let _ = std::fs::write(&interface_config_path, &interface_config);
    
    // Also save as global static.conf (applies to any active interface)
    // This is useful when the user wants static IP regardless of which interface is active
    let global_config_path = format!("{}/static.conf", config_dir);
    let _ = std::fs::write(&global_config_path, &interface_config);
    
    // Save to PERSISTENT storage (survives reboots)
    // The /quantix directory is on the QUANTIX-CFG partition
    let persistent_dir = "/quantix/network";
    if std::path::Path::new("/quantix").exists() {
        let _ = std::fs::create_dir_all(persistent_dir);
        let persistent_interface_path = format!("{}/{}.conf", persistent_dir, config.interface);
        let _ = std::fs::write(&persistent_interface_path, &interface_config);
        let persistent_global_path = format!("{}/static.conf", persistent_dir);
        let _ = std::fs::write(&persistent_global_path, &interface_config);
    }
    
    // Also save to /etc/network/interfaces for legacy compatibility
...
fn render_cluster_screen(f: &mut Frame, app: &App, area: Rect) {
    let mut lines = vec![
        Line::from(Span::styled("🔗 Cluster Registration", Style::default().add_modifier(Modifier::BOLD))),
        Line::from(""),
    ...
    if !matches!(app.cluster_config.status, ClusterStatus::Connected) {
        lines.push(Line::from(Span::styled("📋 Registration Token", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))));
        lines.push(Line::from(""));
    ...
    lines.push(Line::from(Span::styled("  2. In vDC UI, go to Hosts → Add Host", Style::default().fg(Color::DarkGray))));
